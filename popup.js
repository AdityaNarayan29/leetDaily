...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const onboardingResult = await new Promise(r => chrome.storage.local.get(['onboardingComplete'], r));
  if (!onboardingResult.onboardingComplete) {
    ...
    // Step 1 → Step 2
    document.getElementById('onboard-signin').addEventListener('click', () => {
      setTimeout(() => goToStep(step2), 500);
    });
    document.getElementById('onboard-skip-1').addEventListener('click', () => goToStep(step2));

    // Step 2 → Step 3 (save focus choices)
    function saveOnboardingFocus() {
      const req = {
        blind75: document.getElementById('onboard-blind75').checked,
        neetcode150: document.getElementById('onboard-nc150').checked,
        dailyChallenge: document.getElementById('onboard-daily').checked,
        leetcode75: false,
        anySubmission: false,
        companyFocus: false,
        selectedCompanies: [],
    ...
    document.getElementById('onboard-skip-2').addEventListener('click', () => goToStep(step3));
    ...
    // Don't block the rest of popup initialization
  }
